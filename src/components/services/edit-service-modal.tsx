...
export function EditServiceModal({ service, open, onOpenChange }: EditServiceModalProps) {
    ...
    const mutation = useMutation({
        ...
        onSuccess: () => {
            ...
            toast.success("Service record updated! / تم تحديث سجل الخدمة بنجاح")
            onOpenChange(false)
        },
        onError: (error: Error) => {
            toast.error(error.message || "Failed to update service / فشل تحديث السجل")
        },
    })
            ...
            <DialogContent className="sm:max-w-[600px] bg-slate-900 border-slate-800 text-white max-h-[90vh] overflow-hidden flex flex-col">
                <DialogHeader>
                    <DialogTitle className="flex justify-between items-center">
                        <span>Edit Service Record</span>
                        <span dir="rtl">تعديل سجل الخدمة</span>
                    </DialogTitle>
                    <DialogDescription className="text-slate-400 flex justify-between">
                        <span>Modify details of an existing repair or coding job.</span>
                        <span dir="rtl">تعديل تفاصيل عملية إصلاح أو برمجة سابقة.</span>
                    </DialogDescription>
                </DialogHeader>
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-2">
                            ...
                            <DialogFooter className="mt-6">
                                ...
                                >
                                    {mutation.isPending ? "Updating..." : "Update Record / تحديث السجل"}
                                </Button>
                            </DialogFooter>
                        </form>
                    </Form>
